...
import { useMilestoneLikes, useToggleLike, useMilestoneLikers } from "@/hooks/use-social";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
function LikersList({ childId, milestoneId }: { childId: number; milestoneId: number }) {
  const { data: likers, isLoading } = useMilestoneLikers(childId, milestoneId);
      ...
      {count > 0 && (
        <Dialog>
          ...
          <DialogContent className="sm:max-w-xs" onClick={(e) => e.stopPropagation()}>
            ...
            <LikersList childId={childId} milestoneId={milestoneId} />
          </DialogContent>
        </Dialog>
      )}
    </div>
