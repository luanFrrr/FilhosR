        ...
        {showChildSelector ? (
          <div className="flex flex-col">
            ...
            <DropdownMenu>
                ...
                >
                  <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center border border-primary/20 text-primary overflow-hidden">
                    {activeChild?.photoUrl ? (
                      <img
                        src={getTransformedImageUrl(activeChild.photoUrl, {
                          width: 100,
                          resize: "cover",
                        })}
                        alt={activeChild.name}
              ...
              <DropdownMenuContent
                align="start"
                className="w-56 rounded-xl shadow-xl border-border/50 bg-popover/95 backdrop-blur"
              >
                {children?.map((child) => (
                  <DropdownMenuItem
                    key={child.id}
                ...
                <DropdownMenuItem
                  asChild
                  className="cursor-pointer py-3 text-primary font-medium focus:bg-primary/5 rounded-lg"
                >
